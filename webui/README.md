# SWE-Lego-Live-SFT Training Dashboard
...
Open <http://localhost:8091>. By default it reads `../saves` (runs) and
...
The script auto-discovers `cloudflared` from `PATH`. Override with
`CLOUDFLARED_BIN=/path/to/cloudflared` if installed elsewhere. Its output is
redirected to `/tmp/dashboard-tunnel-<port>.log` (override with
...
python server.py --port 8091 --save-dir ../saves --log-dir ../logs --static-dir dist
```
...
python server.py --port 8091 --save-dir ../saves --log-dir ../logs
...
python server.py --port 8091 --save-dir ../saves --wandb-entity "$WANDB_ENTITY" \
...
python server.py [OPTIONS]
  ...
  --save-dir DIR           Directory containing saves/<run>/ output dirs (default: ../saves)
  --extra-save-dir DIR     Additional saves dirs (repeatable)
  --log-dir DIR            Directory with train_*.log console logs (default: ../logs)
...
|---|---|---|
| `SAVE_DIR` | `../saves` | Run output directories |
| `LOG_DIR` | `../logs` | Raw console logs (`train_*.log`) |
